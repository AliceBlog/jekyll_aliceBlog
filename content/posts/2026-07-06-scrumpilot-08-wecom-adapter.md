---
title: "ScrumPilot 08：WeCom Adapter"
subtitle: "复用统一消息协议，把第二个 IM 渠道接进来。"
date: 2026-07-06T10:08:00+08:00
url: "/2026/07/06/scrumpilot-08-wecom-adapter.html"
categories: ["技术"]
tags: ["ScrumPilot", "AI Scrum Master", "Agent工程", "敏捷开发", "从0到1", "企业微信"]
weight: 0
---

Sprint 06 验证了 Adapter 模式是否真的可复用。企业微信和飞书的回调、消息格式、群 @ 机制都不同，但进入系统后应变成同一种 UnifiedMessage。

## 本篇要解决的问题

这一篇关注 `企业微信`。如果把 ScrumPilot 看成一个从 0 到 1 的 AI 工程项目，这一步的价值不是单点功能，而是让上一阶段的能力能够继续向后演进。
...
做第二个 Adapter 的意义很大：它会逼你发现哪些逻辑被错误地写死在飞书里。只有抽出来，ScrumPilot 才能成为多渠道系统。
...
企业微信 是 ScrumPilot 逐步产品化的一块拼图。每一块单独看都不夸张，但组合起来，就能把一个 AI Scrum Master 从 Demo 推到可运行、可观测、可交付的系统。
